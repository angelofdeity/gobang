...
def check_h(states, player, i, j, n=SIZE):
    ...
    win_pos = []
    win_counter = 0
    for pos in range(j-1, -1, -1):  # Check left of the position
        if states[i][pos] == player:
            win_counter += 1
            win_pos.append((i, pos))
        else:
            break
    for pos in range(j+1, n):  # Check right of the position
        if states[i][pos] == player:
            win_counter += 1
            win_pos.append((i, pos))
        else:
...
def check_135_degree(states, player, i, j, n=SIZE):
    ...
    # Check upwards-left direction
    row, col = i - 1, j - 1
    while row >= 0 and col >= 0:
        if states[row][col] == player:
            counter += 1
            win_pos.append((row, col))
            row -= 1
            col -= 1
        else:
            break

    # Check downwards-right direction
    row, col = i + 1, j + 1
    ...
    if counter >= WIN_MIN_COUNT:
        for row, col in win_pos:
            states[row][col] = player + OFFSET
        return True
...
def test_45():
    board = create_board(state=EMPTY)
    print()
    ...
    board[2][8] = 1
    board[5][5] = EMPTY
    display_board((board, 19, SYMBOLS))

    result = check_winning(board, 0, [6, 6], n=SIZE)
    print(result)
